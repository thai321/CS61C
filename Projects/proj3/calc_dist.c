...
int get_min_dist(unsigned char *image, 
    ...
    unsigned int min_dist = squared_Euclidean(im, template , t_width);

    flip_horizontal(template, t_width);
    int temp = squared_Euclidean(im, template, t_width);
    if(temp < min_dist)
    	min_dist = temp;

    for(i = 0; i < 3; i++) {
    	transpose(template,t_width);
    	int temp2 = squared_Euclidean(im, template, t_width);
    	if(temp2 < min_dist)
    		min_dist = temp2;

    	flip_horizontal(template, t_width);
    	temp2 = squared_Euclidean(im, template, t_width);
    	if(temp2 < min_dist)
    		min_dist = temp2;
    }
	return min_dist;
